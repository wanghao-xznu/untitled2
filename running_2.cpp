...
Running_2 *g_running_2=NULL;


extern QString arr_setting[5][6];
void Running_2::Page_init()
{
...
Running_2::Running_2(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::Running_2)
{
...
void Running_2::close_serial()//close
{
    ::close(m_fd);
}
bool Running_2::send_serial(QString str)
{
 ...
 void Running_2::remoteDataIncoming_serial()
 {
     char c;
...
void Running_2::on_pushButton_4_clicked()//返回按钮
{
    duanwei=1;
    timer->stop();
    send_serial("STP");
...
void Running_2::on_pushButton_clicked()//开始运行button
{
    //启动定时器
...
void Running_2::on_pushButton_2_clicked()//暂停运行button
{
    //停止定时器
    timer->stop();
    send_serial("STP");
...
void Running_2::on_pushButton_3_clicked()
{
    //终止
    timer->stop();
    qDebug()<<"STOP";
